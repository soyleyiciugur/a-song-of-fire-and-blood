...
const westRiverEdge = [[170,774],[192,783],[213,794],[227,809],[241,823],[251,842],[267,854],[290,863],[310,876],[319,891],[331,910],[352,924],[381,926],[410,934],[438,937],[472,937]];
...
const reachStormEdge = [[509,1036],[497,1054],[481,1073],[466,1091],[449,1115],[442,1138],[432,1160],[417,1181],[399,1198],[381,1212]];
const dorneEdge = [[0,1446],[173,1446],[176,1381],[186,1364],[198,1348],[212,1327],[224,1304],[235,1282],[250,1268],[267,1255],[275,1238],[293,1232],[312,1238],[331,1239],[349,1228],[365,1224],[381,1212],[402,1204],[423,1199],[448,1181],[466,1158],[489,1140],[513,1118],[536,1105],[551,1122],[555,1148],[553,1170],[540,1195],[535,1230],[560,1255],[640,1265],[800,1265]];
...
const regions = [
  ...
  { id:'the-riverlands', name:'The Riverlands', house:'tully', label:[365,765], size:76,
    ring:[...northEdge.slice(0,-1),...valeEdge.slice(1,-1),...crownRiverEdge.slice(1),...reverse(westRiverEdge).slice(1),[0,774]] },
  { id:'the-westerlands', name:'The Westerlands', house:'lannister', label:[226,928], size:83,
    ring:[[0,771],[170,774],...westRiverEdge.slice(1,14),...reverse(westReachEdge).slice(1)] },
  ...
  { id:'the-stormlands', name:'The Stormlands', house:'baratheon', label:[571,1164], size:80,
    ring:[...crownStormEdge,[800,1265],...reverse(dorneEdge.slice(16)).slice(1),...reverse(reachStormEdge).slice(1)] },
  ...
  { id:'the-reach', name:'The Reach', house:'tyrell', label:[300,1100], size:92,
    ring:[...westReachEdge,...westRiverEdge.slice(14),...crownReachEdge.slice(1),...reachStormEdge.slice(1),...reverse(dorneEdge.slice(0,17)).slice(1)] },
];
...
function trace(mask) {
  ...
  while(edges.size) {
    ...
    pts.push(pts[0]);loops.push(simplify(pts));
  }
  return loops.map(pts=>'M'+pts.map(([x,y])=>`${crop.left+x*2},${crop.top+y*2}`).join('L')+'Z').join('');
}
(async()=>{
  ...
  const result=regions.map((r,i)=>({id:r.id,name:r.name,house:r.house,color:houses.find(h=>h.id===r.house).color,
    ...
    label:{x:Math.round(reference.left+r.label[0]*reference.width/800),y:Math.round(reference.top+r.label[1]*reference.width/800),size:Math.round(r.size*reference.width/800)},path:trace(masks[i])}));
