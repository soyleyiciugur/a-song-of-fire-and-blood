...
await as(a);
const guestReply=(await db.query("insert into public.profile_guestbook(profile_id,author_id,body,parent_id) values($1,$2,'Welcome, friend',$3) returning id",[a,a,guest])).rows[0].id;
assert.equal((await db.query('select parent_id from public.profile_guestbook where id=$1',[guestReply])).rows[0].parent_id,guest);
assert.equal((await db.query('delete from public.profile_guestbook where id=$1 returning id',[guest])).rows.length,1);
assert.equal((await db.query('select id from public.profile_guestbook where id=$1',[guestReply])).rows.length,0,'Deleting a guestbook entry cascades to its replies');
