...
const sharedPage = Buffer.from(JSON.stringify({title:'The Test Chronicle',href:'/test-chronicle'})).toString('base64url');
const initial = Array.from({length:60}, (_,i)=>({id:`00000000-0000-4000-8001-${String(i).padStart(12,'0')}`,conversation_id:conversation,sender_id:i%2?me:partner,body:i===0?'[[media:test-image]]':i===1?'[[reel:test-reel]]':i===2?`[[page:${sharedPage}]]`:`Raven ${i}: A letter from the northern keep.`,created_at:new Date(Date.UTC(2026,8,10,10,i)).toISOString(),edited_at:null,deleted_at:null}));
await mkdir(fixture);
...
server.stdout.on('data',d=>logs+=d); server.stderr.on('data',d=>logs+=d);
  ...
  for (const viewport of [{width:1440,height:900},{width:390,height:844},{width:320,height:568}]) {
    ...
    for(const category of ['Images','Reels','Links']) await page.getByRole('tab',{name:new RegExp(`^${category}\\s+1$`)}).waitFor();
    await page.getByRole('tab',{name:/^Reels\s+1$/}).click();
    await page.getByText('Shared Gutter Reel',{exact:true}).waitFor();
    await page.getByRole('tab',{name:/^Links\s+1$/}).click();
    await page.getByRole('link',{name:/The Test Chronicle/}).waitFor();
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false,'Shared-content hub does not overflow');
