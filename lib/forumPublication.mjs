export function publishForum(forum, chapters, now) {
  ...
  for(const comment of forum.comments){
    ...
    comments.push({id,entryId,authorId,parentId,body,publishedAt,moderation,parentSource,liveThreadId,surface:'forum',upvotes:comment.upvoterIds.length,hasocash:comment.awards.reduce((n,a)=>n+a.amount,0)});
  }
