...
Keep 5–10 distinct ordinary fan accounts per post; the rare character/institution cameos sit outside that fan budget. Validate all identities and replies using `validate:comments`. Browser checks cover pinned root comments, promoted ordinary conversations, canonical profile links, age, non-pinned institutions, and no duplicated comment rendering on mobile and desktop.
