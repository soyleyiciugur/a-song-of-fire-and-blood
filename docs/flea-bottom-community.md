...
3. Read the user profiles (`voice`, `continuity`, public `bio`, and `interests`/`postingStyle` where present) and their existing comments. From 19 September 2026, author at least 15 total comments per new image/video, aim for approximately 25 on average across ordinary posts, and cap new editorial conversations at 50. Replies and persisted future comments count toward the total; this is not a distinct-participant quota or a demand to publish everything at once. Vary lengths rather than making every post exactly 25. Vhaemys posts normally attract 35-50. Selection happens once while authoring, never randomly on page load. There is no mandatory core cast: rotate interested accounts and retain short reactions, disagreement, tangents and genuine reply exchanges. Keep the Gen Alpha account occasional. The explicit scope and historical exception are in `data/community-authoring-policy.json`; do not inflate the whole old archive or cap real member participation.
...
5. Save comments with stable `id`, `entryId`, `authorId`, `parentId`, `body` and an explicit UTC ISO `publishedAt`. Roots use `null`; replies point to an earlier comment in the same post and cannot publish before it. Nested replies retain their full parent chain while the visual indentation becomes progressively compact. Existing conversations are persisted, not regenerated. All NPC accounts can gradually reconsider an opinion, warm to another faction, or develop a personal arc; this is not an exception reserved for a favourite account. Add dated continuity notes tied to the actual exchange. Preserve the old opinion as history and distinguish planned steps from already-published changes. Do not manufacture an overnight conversion to fill a quota.
...
For new conversations, the seven release windows use approximate planning shares of 40 / 20 / 15 / 10 / 5 / 5 / 5 percent. These replace the old fixed per-wave counts, which do not fit an average of 25 and a ceiling of 50. Small waves may have zero comments after rounding; the final conversation still needs at least 15. Expansion of an existing thread preserves its earlier times and adapts to parent availability. Shares are editorial guidance, not a quota enforced by the server or a reason to invent filler.
...
Both notification tabs split their sorted entries into time sections: Now (under five minutes), Earlier this hour, exact elapsed hours within today (for example 3 hours ago or 10 hours ago), Today for older same-day items, Yesterday, then dated sections. Istanbul midnight determines calendar days, and the server snapshot clock advances the headings during normal polling. Keep exact timestamps on entries. Do not backdate archive comments merely to fill every heading. Variation remains additive: all 109 comments removed in an earlier thinning were restored verbatim. Never remove existing comments to create varied counts. The new 15 / approximately 25 / 50 density policy supersedes the earlier sparse-post guidance. By explicit user choice, the existing 57-comment Just a Kid conversation is preserved as an exception and receives no additions in this expansion.
...
The Citadel raven desk, City Watch help desk and Oldtown Visitor Centre use `account: { type: "institution", institutionId }`. `data/gutter-institutions.json` records the institutions and makes clear that their social desks/admins are comedic inventions. Oldtown keeps the existing `regular-brand` ID and comment history; its username alone must never determine verification. All three use gold checks labelled "Verified institution account" in gallery comments and Taverns. Their account metadata must survive the public snapshot. They are occasional staff voices; they do not leak confidential information, issue new canon policies or make civilian victims the joke. Institution-only conversations are not automatically pinned.

Keep thread lengths varied within the current density policy. Most character/institution cameos remain selective; Vhaemys is a deliberate high-frequency exception. Her social-AU screen time is very long: she browses multiple suitable posts, asks literal questions, follows conversations and comes back to a flower, toy, boat or snack she liked. Let her speak for herself as a seven-year-old, rather than only giving adults protective speeches about her. Do not force her into atrocity arguments, adult innuendo or secret-revealing political commentary. Maela's sparse guidance remains unchanged. Validate identities, density and replies using `validate:comments`. Browser checks cover pinned root comments, promoted ordinary conversations, canonical profile links, age, non-pinned institutions, and no duplicated comment rendering on mobile and desktop.
...
## Running jokes and current context

Read [the continuity ledger](community-running-jokes.md) alongside account profiles and the live inbox before extending recurring memes or recruitment banter. Scheduled callbacks are not established public interactions until their publication timestamps.
