...
export default function DragonsPage() {
  return <main className={styles.page}><div className={styles.container}><div className="realm-section-header"><h1 className="realm-page-title">Dragons</h1><p className={styles.subheading}>The living fire of House Targaryen, and the flames that went out too soon.</p><BestiaryTabs active="dragons" /></div>{(["Alive", "Unknown", "Dead"] as const).map((status) => <section className={styles.group} key={status}><h2 className={styles.groupHeading}>{status === "Alive" ? "Living" : status === "Unknown" ? "Unknown" : "Lost"}</h2><div className={styles.grid}>{dragons.filter((dragon) => dragon.status === status).map((dragon) => <DragonCard key={dragon.id} dragon={dragon}/>)}</div></section>)}<BackToBestiary /></div></main>;
}
