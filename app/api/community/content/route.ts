...
export async function POST(request:Request){
 ...
 const input=parsed.data; let result:{error:{message:string}|null};
 ...
 return result.error?NextResponse.json({error:"Your contribution could not be saved."},{status:400}):NextResponse.json({ok:true},{status:201});
}
