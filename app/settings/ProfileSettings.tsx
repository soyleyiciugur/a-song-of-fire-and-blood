...
function MediaPicker({ field, value, onChange }: { field: AffinityField; value: string; onChange: (value: string) => void }) {
  ...
  return <div className={styles.field}>
    ...
    {open && <div className={styles.pickerBackdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><section className={styles.pickerDialog} role="dialog" aria-modal="true" aria-label={`Choose ${field.label}`}><header><div><span>Gallery selection</span><h3>{field.label}</h3></div><button type="button" className={styles.iconButton} onClick={() => setOpen(false)} aria-label="Close gallery">×</button></header><label className={styles.searchBox}><span aria-hidden="true">⌕</span><input value={query} onFocus={(event) => { const input = event.currentTarget; const reveal = () => { const viewport = window.visualViewport; const top = viewport?.offsetTop ?? 0; const bottom = top + (viewport?.height ?? window.innerHeight); const rect = input.getBoundingClientRect(); if (rect.bottom > bottom - 16) window.scrollBy({ top: rect.bottom - (bottom - 16), behavior: "smooth" }); }; reveal(); window.setTimeout(reveal, 180); }} onChange={(event) => { setQuery(event.target.value); setVisible(36); }} placeholder={`Filter ${field.label.toLowerCase()}s by caption…`} /></label><div className={styles.resultCount}>{matches.length} items</div><div className={styles.mediaPicker} role="listbox" aria-label={field.label}>{matches.slice(0, visible).map((option) => <button type="button" role="option" aria-selected={value === option.id} key={option.id} className={`${styles.mediaItem} ${value === option.id ? styles.mediaItemSelected : ""}`} onClick={() => { onChange(option.id); setOpen(false); }}>{option.mediaType === "video" && option.mediaSrc ? <video src={option.mediaSrc} muted playsInline preload="metadata" /> : option.image ? <img src={option.image} loading="lazy" alt="" /> : null}<span>{option.title}</span></button>)}</div>{!matches.length && <p className={styles.noResults}>No gallery items match that caption.</p>}{visible < matches.length && <button className={styles.loadMore} type="button" onClick={() => setVisible((count) => count + 36)}>Show more</button>}</section></div>}
  </div>;
}
