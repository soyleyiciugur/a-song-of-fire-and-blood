...
export default function BestiaryPage() {
  ...
  return <main className={styles.page}><div className={styles.container}>
    <p className={styles.eyebrow}>Creatures Great and Small</p><h1 className="realm-page-title">The Bestiary</h1>
    <p className={styles.lead}>The beasts that run, fly, hunt, and howl through the chronicles.</p>
    <nav className={`${styles.tabs} realm-section-tabs`} aria-label="Bestiary sections">{tabs.map(([href, label]) => <Link aria-current={pathname === href ? "page" : undefined} className={pathname === href ? styles.active : ""} href={href} key={href}>{label}</Link>)}</nav>
