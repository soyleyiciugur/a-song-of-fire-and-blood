...
-- Direct Raven reactions must follow current membership, not only the legacy
-- user_a/user_b pair. Guild Parleys use the same membership table, and the
-- Guild migration backfilled normal two-person Ravens into it as well.
  ...
  select case kind
    ...
    when 'message' then
      exists(
        ...
        join public.direct_raven_members drm on drm.conversation_id=m.conversation_id
