...
export default function AccountControl() {
  ...
  return <details ref={detailsRef} className={styles.accountMenu}><summary aria-label="Account menu"><span>{profile.avatar_url?<img src={profile.avatar_url} alt=""/>:profile.display_name.slice(0,2).toUpperCase()}</span><b>{profile.display_name}</b></summary><div onClick={(event) => { if ((event.target as HTMLElement).closest("a,button") && detailsRef.current) detailsRef.current.open = false; }}><p className={styles.accountIdentity}><strong>{profile.display_name}</strong><small>@{profile.username}</small></p>{state.profileReady?<><Link href={`/users/${profile.username}`}>Profile</Link><Link href="/ledger">My Ledger</Link><Link href="/settings">Settings</Link></>:<p className={styles.profileNotice}>Profile setup pending</p>}<form action={logout}><button>Sign out</button></form></div></details>;
}
