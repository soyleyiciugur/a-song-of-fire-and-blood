...
export default function LikeButton({ kind, id, legacyReactorIds = [] }: { kind:"thread"|"post"|"raven"|"message"; id:string; legacyReactorIds?:string[] }) {
  ...
  return <span className={styles.likeWrap}>
    <button type="button" className={`${styles.reactionButton} ${favor?styles.favorButton:""}`} aria-pressed={liked} aria-label={label} data-tooltip={label} disabled={busy} onClick={()=>void toggle()}>
      {favor ? <svg className={styles.favorIcon} viewBox="0 0 20 20" aria-hidden="true"><path d="m3 10 7-7 7 7h-4v7H7v-7H3Z"/></svg> : <svg viewBox="0 0 24 24" width="18" height="18" fill={liked ? "currentColor" : "none"} aria-hidden="true"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78L12 21.23l8.84-8.84a5.5 5.5 0 0 0 0-7.78Z" stroke="currentColor" strokeWidth="1.55" strokeLinecap="round" strokeLinejoin="round"/></svg>}
    </button>
    ...
    {people&&<span className={styles.reactionOverlay} role="presentation" onPointerDown={event=>{if(event.target===event.currentTarget)setPeople(null);}}><span className={styles.reactionDialog} role="dialog" aria-modal="true" aria-label={favor?"Favor granted by":"Liked by"}><span className={styles.reactionDialogHeader}><b>{favor?"Favor":"Likes"}</b><button type="button" onClick={()=>setPeople(null)} aria-label="Close"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" aria-hidden="true"><path d="M7 7l10 10M17 7 7 17" stroke="currentColor" strokeWidth="1.55" strokeLinecap="round"/></svg></button></span><span className={styles.reactionPeople}>{people.length?people.map((person,index)=>{const content=<><span className={styles.reactorAvatar}>{person.avatar_url?<img src={person.avatar_url} alt=""/>:(person.initials??person.display_name.slice(0,2).toUpperCase())}</span><span><b>{person.display_name}</b><small>@{person.username}</small></span></>;return person.href?<Link key={`${person.username}-${index}`} href={person.href} onClick={()=>setPeople(null)}>{content}</Link>:<span className={styles.reactorRow} key={`${person.username}-${index}`}>{content}</span>}):<span className={styles.noReactions}>No reactions yet.</span>}</span></span></span>}
