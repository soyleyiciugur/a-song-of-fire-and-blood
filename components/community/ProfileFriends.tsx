...
export default function ProfileFriends({ profileId, viewerId }: { profileId: string; viewerId: string | null }) {
  ...
  return <section className={styles.friends}>
    ...
    {viewerId&&!own&&<div className={styles.mutualBlock}><h3>Mutual friends</h3><div className={styles.mutualList}>{people.filter(p=>mutual.includes(p.id)&&rows.some(r=>r.accepted_at&&(r.requester_id===p.id||r.recipient_id===p.id))).map(p=><Link className={styles.mutualChip} key={p.id} href={`/users/${p.username}`}><span className={styles.friendAvatar}>{p.avatar_url?<img src={p.avatar_url} alt=""/>:p.display_name.slice(0,2)}</span><span>{p.display_name}</span></Link>)}</div>{!people.some(p=>mutual.includes(p.id)&&rows.some(r=>r.accepted_at&&(r.requester_id===p.id||r.recipient_id===p.id)))&&<p className={styles.subtle}>No mutual friends yet.</p>}</div>}
