...
export default function InteractiveMap() {
  ...
  const chapters = useMemo(
    () =>
      [...getAllChapters()].sort(
        (a, b) =>
          romanFromTitle(a.title) -
          romanFromTitle(b.title)
      ),
    []
  );
  ...
  const [openCluster, setOpenCluster] =
    useState<string | null>(null);

  const [lockedCluster, setLockedCluster] =
    useState<string | null>(null);

  const [
    activeEventTypes,
    setActiveEventTypes,
  ] = useState<Set<MapEventType>>(
    new Set(ALL_EVENT_TYPES)
  );

  const [travelPos, setTravelPos] =
    useState<{
      xPct: number;
      yPct: number;
    } | null>(null);
  ...
  const resolvedPositionsByChapter =
    useMemo(
      () =>
        resolveCharacterPositionsInOrder(
          orderedSlugs
  ...
  const charactersByLocation =
    useMemo(() => {
      ...
      Object.entries(currentResolved).forEach(
        ([charId, resolved]) => {
          if (!resolved) return;
          ...
          const locs = Array.isArray(location)
            ? location
            : [location];
      ...
      map.forEach((list, key) => {
        list.sort((a, b) => {
          const ca =
            charactersById.get(a.id);

          const cb =
            charactersById.get(b.id);
  ...
  const fullTrailPoints =
    useMemo(() => {
      ...
      for (
        ...
        const positions =
          getCharacterPositionsForChapter(
            chapter.slug
          );

        const location =
          positions[
            selectedCharacterId as keyof typeof positions
          ];
      ...
      return pointsPx
        ...
        .map(
          (
            ...
            return {
              id: `${index}-${startPoint.x.toFixed(
                1
              )}-${startPoint.y.toFixed(
                1
              )}`,
              d: `M ${startPoint.x.toFixed(
                1
              )},${startPoint.y.toFixed(
                1
              )} L ${endPoint.x.toFixed(
                1
              )},${endPoint.y.toFixed(
                1
              )}`,
  ...
  useEffect(() => {
    if (
      ...
      const measuredLength =
        trailPathRef.current.getTotalLength();

      trailPathLengthRef.current =
        measuredLength;
    }
  }, [trailPathD]);

  const selectedCharacterCurrentLocation =
    useMemo(() => {
      if (!selectedCharacterId) {
        return null;
      }
      ...
      const resolved =
        currentResolved[
          selectedCharacterId as keyof typeof currentResolved
        ];
  ...
  useEffect(() => {
    ...
    const naturalSizeForAnimation =
      resolvedNaturalSize;

    const startCenterPxX =
      (rect.width / 2 -
        pan.x) /
      lockedScale;

    const startCenterPxY =
      (rect.height / 2 -
        pan.y) /
      lockedScale;
    ...
    const startXPct =
      (startCenterPxX /
        naturalSizeForAnimation.width) *
      100;

    const startYPct =
      (startCenterPxY /
        naturalSizeForAnimation.height) *
      100;
  ...
  const declutteredLocations =
    useMemo<
      DeclutteredLocation[]
    >(() => {
      if (!naturalSize) {
        return MAP_LOCATIONS.map(
          (loc) => ({
            ...loc,
            offsetXPct:
              loc.xPct,
      ...
      const visible =
        MAP_LOCATIONS.filter(
          (loc) => {
            const hasChars =
              (charactersByLocation.get(
                loc.name
              )?.length ??
                0) > 0;

            const hasEvents =
              (visibleEventsByLocation.get(
                loc.name
              )?.length ??
                0) > 0;
    ...
    >
                  ...
                  return (
                    <button
                      ...
                      className={`${styles.ghostMarker} ${
                        isCurrentChapterStop
                          ? styles.ghostMarkerActive
                          : ""
                      }`}
                      style={
                        {
                        ...
                        {visibleEntries.map(
                          (
                            ...
                            const isPulsing =
                              isSelected &&
                              !entry.faded &&
                              selectedCharacterCurrentLocation?.name ===
                                loc.name;
          ...
          {visibleCard && (
            ...
            >
              ...
              <div>
                ...
                <Link
                  href={`/characters/${visibleCard.id}`}
                  className={
                    styles.characterCardLink
                  }
                >
                  View character →
                </Link>
      ...
      {currentChapter && (
        ...
        >
          ...
          {charactersByLocation.size >
            0 && (
            <div
              className={
                styles.summaryCharacters
              }
            >
              {Array.from(
                charactersByLocation.entries()
              ).map(
                ([
                  location,
                  entries,
                  ...
                  <div
                    ...
                    className={
                      styles.summaryLocationGroup
                    }
                  >
                    <span
                      className={
                        styles.summaryLocationName
                      }
                    >
                      {
                        location
